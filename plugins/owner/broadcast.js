exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (id.length == 0) return client.reply(m.chat, Func.texted('bold', `🚩 Error, ID tidak ada.`), m)
         ...
         if (text) {
            ...
            client.reply(m.chat, Func.texted('bold', `🚩 Berhasil mengirim pesan siaran ke ${id.length} ${command == 'bc' ? 'chats' : 'groups'}`), m)
            ...
            client.reply(m.chat, Func.texted('bold', `🚩 Berhasil mengirim pesan siaran to ${id.length} ${command == 'bc' ? 'chats' : 'groups'}`), m)
         } else client.reply(m.chat, Func.texted('bold', `🚩 Media / teks tidak ditemukan atau media tidak didukung.`), m)
      } catch (e) {
