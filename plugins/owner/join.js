exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!id.endsWith('g.us')) return client.reply(m.chat, Func.texted('bold', `🚩 Maaf bot tidak bisa bergabung dengan grup ini :(`), m)
         let member = await (await client.groupMetadata(id)).participants.map(v => v.id)
         return client.reply(m.chat, `🚩 Joined!`, m)
      } catch {
         return client.reply(m.chat, Func.texted('bold', `🚩 Maaf bot tidak bisa bergabung dengan grup ini :(`), m)
      }
   },
   owner: true
}