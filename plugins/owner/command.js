exports.run = {
   ...
   async: async (m, {
      ...
      if (command == 'disable') {
         if (cmd.error.includes(args[0])) return client.reply(m.chat, Func.texted('bold', `🚩 ${isPrefix + args[0]} perintah sebelumnya dinonaktifkan.`), m)
         ...
         client.reply(m.chat, Func.texted('bold', `🚩 Command ${isPrefix + args[0]} berhasil diaktifkan.`), m)
      }
   },
   owner: true
}