exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         if (!args || !args[0]) {
            if (member.length == 0) return client.reply(m.chat, Func.texted('bold', `🚩 Grup ini bersih dari orang luar.`), m)
            ...
            await client.reply(m.chat, Func.texted('bold', `🚩 Selesai, ${member.length} orang luar berhasil dikick.`), m)
         }
      } catch (e) {
