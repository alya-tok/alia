exports.run = {
   ...
   async: async (m, {
      ...
      if (!input) return client.reply(m.chat, Func.texted('bold', `🚩 Tag atau Reply target obrolan.`), m)
      let p = await client.onWhatsApp(input.trim())
      if (p.length == 0) return client.reply(m.chat, Func.texted('bold', `🚩 Nomor tidak valid.`), m)
      let jid = client.decodeJid(p[0].jid)
      let number = jid.replace(/@.+/, '')
      if (command == 'kick') {
         let member = participants.find(u => u.id == jid)
         if (!member) return client.reply(m.chat, Func.texted('bold', `🚩 @${number} sudah keluar atau tidak ada di grup ini.`), m)
         ...
         let member = participants.find(u => u.id == jid)
         if (member) return client.reply(m.chat, Func.texted('bold', `🚩 @${number} sudah ada di grup ini.`), m)
