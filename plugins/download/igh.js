exports.run = {
   ...
   async: async (m, {
      ...
      try {
         ...
         let old = new Date()
         let json = await Func.fetchJson(API('alya', '/api/igh', { url: Func.igFixed(args[0]) }, 'apikey'))
         ...
         client.sendReact(m.chat, '🕒', m.key)
         let json = await Func.fetchJson(API('alya', '/api/igh2', { username: args[0] }, 'apikey'))
         if (!json.status) return client.reply(m.chat, Func.jsonFormat(json), m)
         json.data.map(async o => {
         let js = await Func.fetchJson(API('alya', '/api/igh2-get', { foid: o.foid }, 'apikey'))
