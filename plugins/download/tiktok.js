  ...
  async: async (m, {
    ...
    try {
      if (/tt|tik(tok|wm|mp3)?/.test(command)) {
        ...
        let old = new Date()
        let json = await Func.fetchJson(API('alya', '/api/tiktok', {
          url: Func.ttFixed(args[0])
        }, 'apikey'))
        if (!json.status)
          return client.reply(m.chat, Func.texted('bold', `🚩 Error! private videos or videos not available.`), m)
        ...
        if (command == 'tiktok' || command == 'tt') {
          ...
          if (command == 'tikwm') return client.sendFile(m.chat, json.data.find(v => v.type == 'watermark').url, Func.filename('mp4'), caption, m)
          if (command == 'tikmp3')
            return !json.music_info.url ? client.reply(m.chat, global.status.fail, m) : client.sendFile(m.chat, json.music_info.url, Func.filename('mp3'), '', m)
        }
      }
    } catch (e) {
      console.log(e)
